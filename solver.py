...
class Solver:
    def __init__(self, args):
        ...
        # Array to record training progression
        self.train_losses     = []
        self.test_losses      = []
    ...
    def test_dataset(self, loader):
        ...
        # Convert all capturde variables to torch
        all_labels = torch.cat(all_labels)
        all_logits = torch.cat(all_labels)
    ...
    def test(self, train=True):
        if train:
            ...
            print(f'Train acc: {acc:.2f}\tTrain loss: {loss:.4f}\nTrain Confusion Matrix:')
            print(cm)


        # Test using test loader
        acc, cm, loss = self.test_dataset(self.test_loader)
        print(f'Test acc: {acc:.2f}\tTest loss: {loss:.4f}\nTest Confusion Matrix:')
        print(cm)        
        return acc, loss
    ...
    def train(self):
        ...
        for epoch in range(self.args.epochs):
            ...
            # Arrays to record epoch less and accuracy
            train_epoch_less = []
            ...
            test_acc, test_loss = self.test(train=((epoch+1)%25==0)) # Test training est every 25 epochs
