...
# K --> Key Sequence lenght (equal to S for self-attention)
...
class EmbedLayer(nn.Module):
    ...
    def forward(self, x):
        B = x.shape[0]  
        # B, C, IH, IW --> B, E, IH/P, IW/P  Split imae into the patches and embed patches
        x = self.conv1(x)    
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x):
        b, s, e = x.shape # Note: In case of self-attention Q, K, V are all equal to S
